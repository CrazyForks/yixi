...
- **The UI is bilingual (Chinese and English)**, switched from the footer, the language section on `/account`, or `?lang=en` / `?lang=zh` on any page; Chinese is the default. `/today/setup` (Guide) is translated along with the rest of this face — the Shortcut walkthrough at `/setup` on the Breathe side is still Chinese only.
