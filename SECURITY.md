...
| Someone with an unlocked phone | `/review` and `/settings` open with no prompt (the session cookie lasts 180 days). The gate token itself needs one more deliberate tap. | Device lock. The token is masked behind `?show=1` on `/setup` and `/account` specifically so a shoulder-surfer does not get it for free. |
...
- **The login redirect cannot be turned into an open redirect.** `?next=` is only honoured for a same-site path; an absolute URL (`//host` included) falls back to `/review`.
